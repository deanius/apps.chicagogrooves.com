      ...
      function(entry) {
        if (entry.path.toLowerCase().indexOf(url) > -1) {
          if (
            ...
            entry.path.indexOf("/_") > -1
          ) {
            //dont show
          } else if (entry.path.indexOf(".youtube") > -1) {
            var content = fs
              .readFileSync("./public/" + entry.path, "UTF8")
              .chomp();
            matches.push({
              name: entry.path,
              path: entry.path,
...
var dynamicHelpers = {
  ...
  url: function(req, res) {
    return function(path) {
      return (
        app.dynamicViewHelpers.scheme(req, res) +
